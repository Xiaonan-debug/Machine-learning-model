...
class DecisionTree():
    ...
    def _create_tree(self, features, targets, used_attributes, default):
        ...
        if (np.all(targets == targets[0])):
            return Node(return_value=targets[0])
        ...
        for i in range(len(self.attribute_names)):
            ...
            if(attribute in used_attributes):
                ...
                all_in = bool(False)
                IG[i] = information_gain(features=features, attribute_index=i, targets=targets)
        
        if(all_in):
            return Node(return_value=find_mode(targets))
        
        all_values = IG.values()
        largest = max(all_values)
        best_index = -1
        if(largest >= 0):
            for atr, ig in IG.items():
                if(ig == largest):
                    best_index = atr
                    break
        
        attribute_values = features[:, best_index]
        ...
        if(is_binary):
            ...
            split_value = statistics.median(attribute_values)

        attribute_name = self.attribute_names[best_index]
        attribute_index = best_index
        used_attributes = np.append(used_attributes, attribute_name)
        node = Node(split_value=split_value, attribute_name=attribute_name, attribute_index=attribute_index)
        group_1 = []
        group_2 = []
        target_1 = np.array([])
        target_2 = np.array([])
        for i in range(len(attribute_values)):
            ...
            f = features[i]
            if(attribute < split_value):
                group_1.append(f)
                target_1 = np.append(target_1, targets[i])
            else:
                group_2.append(f)
                target_2 = np.append(target_2, targets[i])
        group_1 = np.array(group_1)
        group_2 = np.array(group_2)
        node.branches = np.append(node.branches, self._create_tree(features=group_1, targets=target_1, used_attributes=used_attributes, default=default))
        node.branches = np.append(node.branches, self._create_tree(features=group_2, targets=target_2, used_attributes=used_attributes, default=default))
        return node
    ...
    def predict(self, features):
        ...
        cols = 1
        rows = len(features)
        predictions = [[0]*cols]*rows
        root = self.tree
        for i in range(len(features)):
            e = features[i]
            while(self.tree.return_value == None):
                root_index = self.tree.attribute_index
                if(e[root_index] <= self.tree.split_value):
                    self.tree = self.tree.branches[0]
                else:
                    self.tree = self.tree.branches[1]

            predictions[i] = [self.tree.return_value]
            self.tree = root
...
def information_gain(features, attribute_index, targets):
    ...
    if(np.isin(attribute_values, [0, 1]).all()):
            split_value = 0.5
    else:
        split_value = statistics.median(attribute_values)
    group_1 = []
    ...
    for i in range(len(attribute_values)):
        attribute = attribute_values[i]
        if(attribute < split_value):
            group_1 = np.append(group_1, attribute)
            target_1 = np.append(target_1, targets[i])
        else:
            group_2 = np.append(group_2, attribute)
            target_2 = np.append(target_2, targets[i])

    S_entropy = entropy(targets)
    gp1_entropy = entropy(target_1)
    gp2_entropy = entropy(target_2)
    return S_entropy - (len(group_1)*gp1_entropy)/len(attribute_values) - (len(group_2)*gp2_entropy)/len(attribute_values)
