...
def softmax(x, axis=1):
    ...
    """
    x = np.atleast_2d(x)
    x_new = np.amax(x, axis=axis, keepdims=True)

    x_nn = np.exp(x - x_new)
    return x_nn / np.sum(x_nn, axis=axis, keepdims=True)


def stable_log_sum(a):
    """
    ...
    assert a.shape[1] == 2 and len(a.shape) == 2
    sum = 0
    for i in range(a.shape[0]):
        sum += np.logaddexp(a[i,0],a[i,1])
    return sum
