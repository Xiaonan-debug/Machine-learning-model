...
def test_softmax():
    ...
    assert np.allclose(softmax(small, axis=1), np.array([0.25, 0.75]))
    assert np.allclose(softmax(small, axis=0), np.array([1, 1]))
    ...
    assert np.allclose(softmax(X, axis=0), exp0)
    assert np.allclose(softmax(X, axis=1), exp1)
